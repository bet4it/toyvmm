...
use crate::utils::util::u64_to_usize;
...
pub struct FirecrackerTarget {
    ...
    pub gdb_event: Receiver<(usize, Sender<()>)>,
...
impl FirecrackerTarget {
    ...
    pub fn new(
        guest_memory: GuestMemoryMmap,
        vcpu_fds: Vec<VcpuFd>,
        gdb_event: Receiver<(usize, Sender<()>)>,
        entry_addr: GuestAddress,
    ) -> Self {
    ...
    pub fn get_stop_reason(
        ...
        let gpa = arch::translate_gva(&vcpu_state.vcpu_fd, ip, &self.guest_memory)?;
...
impl MultiThreadBase for FirecrackerTarget {
    ...
    fn read_addrs(
        ...
        while !data.is_empty() {
            let gpa = arch::translate_gva(&vcpu_state.vcpu_fd, gva, guest_memory).map_err(|e| {
...
impl SwBreakpoint for FirecrackerTarget {
    ...
    fn add_sw_breakpoint(
        ...
        let gpa = arch::translate_gva(&self.get_paused_vcpu()?.vcpu_fd, addr, &self.guest_memory)?;
